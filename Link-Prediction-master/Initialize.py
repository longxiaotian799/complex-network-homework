...
def Data_Shape(Data):
    ...
    length_A = len(List_A)
    length_B = len(List_B)
    print(f'    数据集长度：{str(Data.shape[0])}')
    print(f'    第一列节点长度：({str(length_A)})')
    print(f'    第二列节点长度：({str(length_B)})')
    MaxNodeNum =  int(max(max(List_A),max(List_B))) + 1
    print(f'    节点数量为：{str(MaxNodeNum)}')
    return MaxNodeNum
...
def writeTrainFile(Matrix,NetName):
    ...
    with open(f'Data\\{NetName}\\Train.txt','w') as file:
        np.savetxt(file,index,fmt='%d')

def writeTestFile(Matrix,NetName):
    Matrix = np.triu(Matrix)
    index = np.argwhere(Matrix != 0)
    with open(f'Data\\{NetName}\\Test.txt','w') as file:
        np.savetxt(file,index,fmt='%d')
...
def Divide(NetFile,MatrixAdjacency_Net,MaxNodeNum,NetName):
    print("Divide......")
    DivideTime_Start = time.process_time()
    DivideNum = 0.9
    NetData = np.loadtxt(NetFile)
    ...
    print(f'    训练集邻接矩阵：{str(MatrixAdjacency_Train.shape)}')
    print(f'    训练集计划边数：{str(lengthData - NumTest)}')
    print(f'    训练集实际边数：{str((np.nonzero(MatrixAdjacency_Train)[0].shape[0])//2)}')
    print(f'    测试集邻接矩阵：{str(MatrixAdjacency_Test.shape)}')
    print(f'    测试集计划边数：{str(NumTest)}')
    print(f'    测试集实际边数：{str((np.nonzero(MatrixAdjacency_Test)[0].shape[0])//2)}')

    writeTrainFile(MatrixAdjacency_Train, NetName)
    writeTestFile(MatrixAdjacency_Test, NetName)
    DivideTime_End = time.process_time()
    print(f'DivideTime：{(DivideTime_End - DivideTime_Start)} s')
